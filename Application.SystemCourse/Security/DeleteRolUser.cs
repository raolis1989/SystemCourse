    ...
    public class DeleteRolUser
    {
        ...
        public class Handler : IRequestHandler<Eject>
        {
            ...
            public async Task<Unit> Handle(Eject request, CancellationToken canccelationToken)
            {
                ...
                if (role == null)
                {
                    throw new HandlerException(HttpStatusCode.NotFound, new { messaje = "Rol Not Find" });
                }

                var userIden = await _userManager.FindByNameAsync(request.UserName);
                if (userIden == null)
                {
                    throw new HandlerException(HttpStatusCode.NotFound, new { messaje = "User Not Find" });
                }
